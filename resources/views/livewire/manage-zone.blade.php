<div>
    ...
    <a href="{{ route('zones.create') }}" title="Új rekord"
    ...
    <div class="flex flex-col">
        ...
        <div class="flex flex-row items-center justify-center space-x-4 mb-4">
            <div class="relative w-3/12">
                ...
                <input type="search" wire:model.debounce.500ms="search.zone_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Keresés zóna nevére ...">
            </div>
        </div>
        ...
        <x-table>
            <x-slot name="head">
                ...
                <x-table.heading class="w-3/12">
                    <div class="flex items-center">
                        <button wire:click="sortBy('location.name')"
                                class="leading-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Kapcsolódó lokáció(k) neve
                        </button>
                        <x-table.sort-icon field="location.name" :sortField="$sortField" :sortDirection="$sortDirection" />
                    </div>
                </x-table.heading>
                ...
                @if ($selectPage)
                    <x-table.row class="bg-gray-200" wire:key="row-message">
                        <x-table.cell colspan="5" class="bt-0">
                            @unless($selectAll)
                                <div class="text-sm">
                                    <span>Kiválasztott rekordok száma: <strong>{{ $zones->count() }}</strong> db., ki szeretnéd mind a(z) <strong>{{ $zones->total() }}</strong> db.-ot választani?</span>
                                    <a href="#" wire:click="$set('selectAll', true)" class="ml-1 text-blue-600">Összes rekord kiválasztása</a>
                                </div>
                            @else
                                <span class="text-sm">Jelenleg mind a(z) <strong>{{ $zones->total() }}</strong> rekordot kiválasztottad.</span>
                            @endif
                        </x-table.cell>
                    </x-table.row>
                @endif
                    @forelse($zones as $zone)
                        @forelse($zone->locations as $location)
                            <x-table.row wire:loading.class.delay="opacity-75" wire:key="row-{{ $zone->id }}-{{ $location->id }}">
                                @if ($loop->first)
                                    <x-table.cell>
                                        <input type="checkbox" wire:model="selectedItems" name="selectedItems" value="{{ $zone->id }}" />
                                    </x-table.cell>
                                    <x-table.cell>
                                        <div class="flex items-center">
                                    ...
                                    <x-table.cell class="text-right text-sm font-medium space-x-2">
                                        <a type="button" href="{{ route('zones.edit', $zone->id) }}" class="px-3 py-3 bg-blue-500 hover:bg-blue-600 text-white text-xs rounded-md">
                                            <i class="fas fa-edit fa-sm" aria-hidden="true" title="Edit"></i>
                                        </a>
                                        <button type="button" wire:click="confirmDelete({{ $zone->id }})" class="px-3 py-3 bg-red-500 hover:bg-red-600 text-white text-xs rounded-md">
                            ...
                            <x-table.row wire:key="row-empty">
                                <x-table.cell colspan="5">
                                    <div class="flex justify-center items-center">
                                        <span class="py-8 text-base font-medium text-gray-400 uppercase">Nincsen ilyen zóna ...</span>
                                    </div>
                                 </x-table.cell>
                            </x-table.row>
                    @endforelse
    ...
    <form method="POST" wire:submit.prevent="delete">
        ...
        <x-modals.delete wire:model.defer="showDeleteModal">
            @if ($selectedItems)
                <x-slot name="title">Zónák törlése</x-slot>
            @else
                <x-slot name="title">Zóna törlése - {{ $currentZone->name }}</x-slot>
            @endif
            <x-slot name="body">
                @if ($showDeleteModal && $selectedItems )
                    Biztos benne, hogy törölni kívánja a kiválasztott zónákat? A művelet nem visszavonható
                    változást eredményez a rendszer adatbázisában.
                    <label for="message" class="block mt-2 mb-2 text-sm text-gray-900">
                        A törölni kívánt zónák listája (első 10 db.):
                    </label>
                    <textarea id="message"
                              rows="4"
                    ...
                    >@foreach (collect($selectedItems)->take(10) as $item){{ trim($zones->firstWhere('id', $item)->name).PHP_EOL }}@endforeach</textarea>
                @else
                    Biztos benne, hogy törölni kívánja a kiválasztott zónát? A művelet nem visszavonható
                    változást eredményez a rendszer adatbázisában.
                @endif
            </x-slot>
            <x-slot name="footer">
