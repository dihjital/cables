<div class="float-right ml-2">
    ...
    <form wire:submit.prevent="import" method="POST" enctype="multipart/form-data">
        ...
        <x-modals.import model="showImportModal">
            <x-slot name="title">{{ __('Kábelek betöltése') }}</x-slot>
            <x-slot name="body">
                @unless($upload)
                <p class="text-sm text-gray-500 break-normal">{{ __('Válassza ki a betölteni kívánt állományt a gépéről. Az
                    állománynak az előre megadott formátummal kell rendelkeznie a sikeres adatbetöltéshez.') }}</p>
                ...
                @else
                    @unless(count($importFailures))
                        <x-forms.import-select>
                            ...
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.full_name" name="fieldColumnMap.full_name">
                                    <option wire:key="key-full_name" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                        ...
                        <x-forms.import-select>
                            <x-slot name="label">
                                <x-forms.label name="fieldColumnMap.startCD" :label="__('Kezdő eszköz')"></x-forms.label>
                            </x-slot>
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.startCD" name="fieldColumnMap.startCD">
                                    <option wire:key="key-startCD" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                                    @foreach($columns[0][0] as $column)
                        ...
                        <x-forms.import-select>
                            ...
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.start" name="fieldColumnMap.start">
                                    <option wire:key="key-start" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                                    @foreach($columns[0][0] as $column)
                        ...
                        <x-forms.import-select>
                            <x-slot name="label">
                                <x-forms.label name="fieldColumnMap.endCD" :label="__('Végződő eszköz')"></x-forms.label>
                            </x-slot>
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.endCD" name="fieldColumnMap.endCD">
                                    <option wire:key="key-endCD" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                                    @foreach($columns[0][0] as $column)
                        ...
                        <x-forms.import-select>
                            ...
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.end" name="fieldColumnMap.end">
                                    <option wire:key="key-end" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                                    @foreach($columns[0][0] as $column)
                        ...
                        <x-forms.import-select>
                            ...
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.i_time" name="fieldColumnMap.i_time">
                                    <option wire:key="key-i_time" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                                    @foreach($columns[0][0] as $column)
                        ...
                        <x-forms.import-select>
                            ...
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.status" name="fieldColumnMap.status">
                                    <option wire:key="key-status" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                                    @foreach($columns[0][0] as $column)
                        ...
                        <x-forms.import-select>
                            ...
                            <x-slot name="select">
                                <x-forms.select wire:model="fieldColumnMap.purpose" name="fieldColumnMap.purpose">
                                    <option wire:key="key-purpose" value="" disabled selected>{{ __('Kérem válasszon') }}!</option>
                                    @foreach($columns[0][0] as $column)
                    ...
                    @else
                        <div class="flex p-4 mb-1 text-sm text-red-700 bg-red-100 rounded-lg" role="alert">
                            ...
                            <div>
                                <span><h3>{{  __('Tömeges betöltés sikertelen') }}!</h3></span>
                                <ul class="mt-1.5 ml-4 list-disc list-inside">
                                    ...
                                    <details>
                                        <summary><strong>{{ __('Errors') }}:</strong></summary>
                                    ...
                                    <details>
                                        <summary><strong>{{ __('Values') }}:</strong></summary>
