<div>
    ...
    <form method="POST" wire:submit.prevent="delete">
        ...
        <x-modals.delete wire:model.defer="showDeleteModal">
            @if ($selectedItems)
                <x-slot name="title">Kábelek törlése</x-slot>
            @else
                <x-slot name="title">Kábel törlése - {{ $currentCable->full_name }}</x-slot>
            @endif
            <x-slot name="body">
                @if ($showDeleteModal && $selectedItems )
                    Biztos benne, hogy törölni kívánja a kiválasztott kábeleket? A művelet nem visszavonható
                    változást eredményez a rendszer adatbázisában.
                    <label for="message" class="block mt-2 mb-2 text-sm text-gray-900">
                        A törölni kívánt kábelek listája (első 10 db.):
                    </label>
                    <textarea id="message"
                              rows="4"
                              class="text-left p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                ...
                @else
                    Biztos benne, hogy törölni kívánja a kiválasztott kábelt? A művelet nem visszavonható
                    változást eredményez a rendszer adatbázisában.
                @endif
            </x-slot>
            <x-slot name="footer">
