<div>
    ...
    <div class="flex flex-col">
        ...
        <div class="flex flex-row items-center justify-center space-x-4 mb-4">
            <div class="relative w-3/12">
                ...
                <input type="search" wire:model.debounce.500ms="search.owner_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="{{ __('Search for owner\'s name') }} ...">
            </div>
        </div>
        ...
        <x-table>
            <x-slot name="head">
                ...
                <x-table.heading class="w-3/12">
                    <div class="flex items-center">
                        {{ __('Number of connectivity devices') }}
                    </div>
                </x-table.heading>
                    ...
                    <x-table.row wire:key="row-empty">
                        <x-table.cell colspan="5">
                            <div class="flex justify-center items-center">
                                <span class="py-8 text-base font-medium text-gray-400 uppercase">{{ __('There is no such owner') }} ...</span>
                            </div>
                        </x-table.cell>
                    </x-table.row>
                @endforelse
    ...
    <form method="POST" wire:submit.prevent="delete">
        ...
        <x-modals.delete wire:model.defer="showDeleteModal">
            @if ($selectedItems)
                <x-slot name="title">{{ __('Delete owners') }}</x-slot>
            @else
                <x-slot name="title">{{ __('Delete owner - :Name', ['name' => $currentOwner->name]) }}</x-slot>
            @endif
            <x-slot name="body">
